...
function __summarizeSnapshot(before, after, title, opts){
  try{
    ...
    for(const [name, beforeS] of beforeMap){
      ...
      if(dDS !== 0) changes.push(`语文 ${dDS>0?'+':''}${dDS}`);
      if(dGraph !== 0) changes.push(`数学 ${dGraph>0?'+':''}${dGraph}`);
      if(dStr !== 0) changes.push(`英语 ${dStr>0?'+':''}${dStr}`);
      if(dMath !== 0) changes.push(`物理 ${dMath>0?'+':''}${dMath}`);
      if(dDP !== 0) changes.push(`道法 ${dDP>0?'+':''}${dDP}`);
...
function simulateHiddenMockScore(s, diffIdx){
  const knowledge_types = ["语文","数学","英语","物理","道法"];
  let total = 0;
...
const KP_OPTIONS = [{id:1,name:"语文"},{id:2,name:"数学"},{id:3,name:"英语"},{id:4,name:"物理"},{id:5,name:"道法"}];
...
function initGame(difficulty, province_choice, student_count){
  ...
  for(let i=0;i<student_count;i++){
    ...
    let aaa = clamp(normal(mean, stddev), 0, 100);
    let bbb = clamp(normal(mean, stddev), 0, 100);
    let ccc = clamp(normal(mean, stddev), 0, 100);
    let ddd = clamp(normal(mean, stddev), 0, 100);
    let eee = clamp(normal(mean, stddev), 0, 100);
    const newStud = new Student(name, thinking, coding, mental,aaa,bbb,ccc,ddd,eee);
    try{ if(window.TalentManager && typeof window.TalentManager.assignInitialTalent === 'function') window.TalentManager.assignInitialTalent(newStud); }catch(e){}
    game.students.push(newStud);
  }
  game.updateWeather();
