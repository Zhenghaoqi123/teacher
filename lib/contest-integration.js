...
(function(global){
  ...
  const compOrder = window.COMPETITION_ORDER || ["限时作业","期中考试","期末考试","中考选拔","中考"];
   ...
   * @param {Array} internationalStudents - 国际选手列表（仅IOI）
    ...
    const results = [];
      for(let state of studentStates){
        ...
        let medal = null;
        if(originalComp && originalComp.name === '中考'){
  ...
  const halfIndex = (game.week > halfBoundary_local) ? 1 : 0;
    ...
    if(originalComp && originalComp.name === '中考' && halfIndex === 1){
  ...
  // 显示为两个维度的难度：思维难度 / 代码难度（使用该题最后一档的值代表题目总体难度）
        ...
        problemHeaders += `<th style="font-size:11px;">
          T${prob.id+1}<br/>
          <span style="color:#888;">思维:${thinking} / 书写:${tcoding}</span><br/>
    ...
    participatedResults.forEach((r, index) => {
      ...
      for(let prob of r.problems){
        ...
        const isNOIOrAbove = compName === '中考' || compName === 'IOI';
    ...
    try{
      if(typeof window.closeModal === 'function'){
        ...
        window.closeModal = function(){
          ...
          if(hasGoldMedal && config && config.name === '中考'){
            try{
   ...
   * 处理模拟赛结果
    ...
    for(let state of studentStates){
      ...
      if(config.contestType === 'online'){
        ...
        if(diffIdx <= 0){
          gainRatio = CONTEST_GAIN_RATIOS['限时作业'] || gainRatio;
        } else if(diffIdx === 1){
          gainRatio = CONTEST_GAIN_RATIOS['期中考试'] || gainRatio;
        } else if(diffIdx === 2){
          gainRatio = CONTEST_GAIN_RATIOS['期末考试'] || gainRatio;
        } else {
          gainRatio = CONTEST_GAIN_RATIOS['中考选拔'] || gainRatio;
        }
      }
      ...
      if(window.__OI_DEBUG_MOCK_GAINS){
        ...
        console.log(`  书写上限:${codingCap.toFixed(2)}, 分配:`, codingGains);
      ...
      for(let i = 0; i < state.problems.length; i++){
        ...
        for(let tag of tags){
          if(typeof s.addKnowledge === 'function'){
            ...
            // 兼容：直接修改字段
            if(tag === '语文') s.knowledge_ds = (s.knowledge_ds || 0) + perTagGain;
            if(tag === '数学') s.knowledge_graph = (s.knowledge_graph || 0) + perTagGain;
            if(tag === '英语') s.knowledge_string = (s.knowledge_string || 0) + perTagGain;
            if(tag === '物理') s.knowledge_math = (s.knowledge_math || 0) + perTagGain;
            if(tag === '道法' || tag === 'DP') s.knowledge_dp = (s.knowledge_dp || 0) + perTagGain;
          }
        }
      }
      ...
      const kDs = after.knowledge_ds - before.knowledge_ds; if(kDs) deltas.push(`语文 ${kDs>0?'+':''}${kDs}`);
      const kG = after.knowledge_graph - before.knowledge_graph; if(kG) deltas.push(`数学 ${kG>0?'+':''}${kG}`);
      const kS = after.knowledge_string - before.knowledge_string; if(kS) deltas.push(`英语 ${kS>0?'+':''}${kS}`);
      const kM = after.knowledge_math - before.knowledge_math; if(kM) deltas.push(`物理 ${kM>0?'+':''}${kM}`);
      const kD = after.knowledge_dp - before.knowledge_dp; if(kD) deltas.push(`道法 ${kD>0?'+':''}${kD}`);
