  ...
  class ContestSimulator {
    ...
    simulateStudentTick(state){
      ...
      if(needsNewTarget){
        ...
        // 添加选题日志
        this.addLog(`${s.name} 开始做第${selected + 1}题`, 'select', s.name);
      ...
      if(subtaskIdxToTry === null){
        this.addLog(`${s.name} 没有可尝试的档位，放弃第 ${state.currentTarget + 1} 题`, 'skip', s.name);
        state.currentTarget = null;
        state.thinkingTime = 0;
        return;
      }
      ...
      if(success){
        ...
        if(isAC){
          this.addLog(`${s.name} 在第 ${state.currentTarget + 1} 题获得了 100 分！`, 'solve', s.name);
        } else {
          this.addLog(`${s.name} 通过了第 ${state.currentTarget + 1} 题的第 ${subtaskIdxToTry + 1} 档，得分：${subtask.score}`, 'info', s.name);
        }
        ...
        if(typeof s.triggerTalents === 'function'){
          ...
          try{
            ...
            if(tags.includes('语文')) knowledgeType = 'knowledge_ds';
            else if(tags.includes('数学')) knowledgeType = 'knowledge_graph';
            else if(tags.includes('英语')) knowledgeType = 'knowledge_string';
            else if(tags.includes('物理')) knowledgeType = 'knowledge_math';
            else if(tags.includes('道法')) knowledgeType = 'knowledge_dp';
        ...
        this.addLog(`${s.name} 尝试第 ${state.currentTarget + 1} 题第 ${subtaskIdxToTry + 1} 档失败`, 'info', s.name);
        ...
        if(shouldSkip){
          this.addLog(`${s.name} 在第 ${state.currentTarget + 1} 题上卡住太久，决定跳题`, 'skip', s.name);
      ...
      if(shouldDowngrade){
        ...
        if(lowerSubtasks.length > 0){
          ...
          if(getRandom() < downgradeProb){
            ...
            // 添加详细日志说明决策
            const reason = cannotSolveFullScore ? '发现 100 分的解法太难' : `卡题${thinkingTime}分钟`;
    ...
    attemptSubtask(student, problem, subtask){
      ...
      try{
        ...
        if(problem && typeof problem.difficulty === 'number' && problem.difficulty > 2.0 * effectiveAbility){
          thinkingProb *= 0.45;
          codingProb *= 0.45;
          const msg = `${student.name} 在第 ${(problem.id||0) + 1} 题触发 难度压制：题目难度 ${problem.difficulty} > 2× 综合能力 ${effectiveAbility.toFixed(1)}，思维/代码通过概率被压制`;
      ...
      try{
        ...
        let logMsg = `${student.name} 尝试第 ${(problem.id||0)+1} 题档位：思维通过 ${thinkingPass} (${tPct}%)，书写通过 ${codingPass} (${cPct}%)`;
    ...
    finish(){
      ...
      const mistakeApplicableContests = ['期末考试', '期中考试', '中考选拔','限时作业','中考'];
      ...
      for(let st of this.students){
        ...
        for(let prob of st.problems){
          ...
          if(getRandom() < mistakeProbability){
            ...
            // 记录失误日志
            this.addLog(`${s.name} 在第 ${prob.id + 1} 题上失误：${reason}，扣除 ${penalty} 分`, 'skip', s.name);
            ...
            try{
              if(typeof window !== 'undefined' && window.pushEvent){
                window.pushEvent({
                  name: '比赛失误',
                  description: `【${this.config.name}】${s.name} 在第 ${prob.id + 1} 题中，由于${reason}，挂分 ${penalty} 分`,
