 ...
 * @returns {Object} - { hasQualification: boolean, nextContest: string, html: string }
  ...
  try {
    ...
    if (nextComp.name === '限时作业') {
      result.hasQualification = true;
      result.html = '<span class="qualification-badge qualified" title="所有学生均可参加限时作业">✓</span>';
      return result;
    }
...
function renderDifficultyTag(diff){
  ...
  let label = '';
  let cls = '';
  if(d <= 20){ label = '轻而易举'; cls = 'diff-red'; }
  else if(d <= 40){ label = '唾手可得'; cls = 'diff-orange'; }
  else if(d <= 60){ label = '信手拈来'; cls = 'diff-yellow'; }
  else if(d <= 80){ label = '略费周章'; cls = 'diff-green'; }
  else if(d <=100){ label = '煞费苦心'; cls = 'diff-blue'; }
  else if(d <= 120){ label = '千难万险'; cls = 'diff-purple'; }
  else { label = '难于登天'; cls = 'diff-black'; }
      ...
      <div class="student-details" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <div class="knowledge-badges">
            <span class="kb" title="语文: ${Math.floor(Number(s.knowledge_ds||0))}" data-grade="${getLetterGradeAbility(Math.floor(Number(s.knowledge_ds||0)))}">
              语文 ${getLetterGradeAbility(Math.floor(Number(s.knowledge_ds||0)))}
            </span>
            <span class="kb" title="数学: ${Math.floor(Number(s.knowledge_graph||0))}" data-grade="${getLetterGradeAbility(Math.floor(Number(s.knowledge_graph||0)))}">
              数学 ${getLetterGradeAbility(Math.floor(Number(s.knowledge_graph||0)))}
            </span>
            <span class="kb" title="英语: ${Math.floor(Number(s.knowledge_string||0))}" data-grade="${getLetterGradeAbility(Math.floor(Number(s.knowledge_string||0)))}">
              英语 ${getLetterGradeAbility(Math.floor(Number(s.knowledge_string||0)))}
            </span>
            <span class="kb" title="物理: ${Math.floor(Number(s.knowledge_math||0))}" data-grade="${getLetterGradeAbility(Math.floor(Number(s.knowledge_math||0)))}">
              物理 ${getLetterGradeAbility(Math.floor(Number(s.knowledge_math||0)))}
            </span>
            <span class="kb" title="道法: ${Math.floor(Number(s.knowledge_dp||0))}" data-grade="${getLetterGradeAbility(Math.floor(Number(s.knowledge_dp||0)))}">
              道法 ${getLetterGradeAbility(Math.floor(Number(s.knowledge_dp||0)))}
            </span>
        ...
        ${talentsHtml ? `<div style="display:flex;align-items:center;gap:6px;"><span style="font-size:12px;color:#718096;font-weight:600;">天赋（点击天赋查看属性）</span><div class="student-talents">${talentsHtml}</div></div>` : ''}
      </div>
    </div>`;
...
function holdMockContestUI(){
  ...
  showModal(`<h3>配置模拟考试（1周）</h3>
    <div><label class="block">比赛类型</label>
      <select id="mock-purchase">
        <option value="0">练习册（免费）</option>
        <option value="1">付费比赛（可选难度和学科）</option>
      </select>
    </div>
  ...
  $('mock-submit').onclick = ()=>{
    ...
    if(isPurchased){
      ...
      game.recordExpense(adj, '购买付费题目');
      log(`购买付费题目（${difficultyConfig.name}）， ¥${adj}`);
    } else {
      log(`完成练习册（${difficultyConfig.name}，免费）`);
    }
...
function entertainmentUI(){
  const opts = [
    ...
    {id:5,label:`邀请学生打游戏`,desc:'适度减压,有可能提升学生能力',cost:ENTERTAINMENT_COST_CS}
  ];
...
function upgradeFacilitiesUI(){
  const facs = [{id:"computer",label:"投影与黑板"},{id:"library",label:"资料库"},{id:"ac",label:"空调"},{id:"dorm",label:"宿舍"},{id:"canteen",label:"食堂"}];
  ...
  try {
    ...
    if (gameData.careerCompetitions && Array.isArray(gameData.careerCompetitions)) {
      for (let comp of gameData.careerCompetitions) {
        if (comp.name === '中考' && comp.entries && Array.isArray(comp.entries)) {
...
function mapEndingToDescription(endingTitle){
  const map = {
    ...
    '🌟 荣耀结局': '队伍取得辉煌胜利，获得中考金牌，你也因此成为金牌老师，学校声誉大增，学生与导师名声大振，未来发展与资源扶持接踵而至。',
